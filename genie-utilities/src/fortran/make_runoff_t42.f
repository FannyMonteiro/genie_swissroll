...
c     I compile with: 
c     ifort -o make_runoff_t42 make_runoff_t42.f -lnc1 -lutil1 -lnetcdf -ligcm3 -L/home/ggpjv/genie/genie-lib/libnc1 -L/home/ggpjv/genie/genie-lib/libutil1 -L/home/ggpjv/genie/genie-igcm3/lib -L/opt/local/lib -fpp
...
c     Eventually, this piece of code could run with makefile.arc!
      ...
      implicit none
      include '/home/ggpjv/genie/genie-igcm3/src/fortran/netcdf.inc'
      include '/home/ggpjv/genie/genie-igcm3/src/fortran/netdata.cmn'
c#include "/home/ggpjv/genie/genie-igcm3/src/fortran/param1.cmn"
c      include '/home/ggpjv/genie/genie-igcm3/src/fortran/param2.cmn'

c     This is the standard land-sea mask:
c      include 'names_runoff.inc' 

c     This is the standard land-sea mask:
      include 'names_runoff_t42.inc' 
...
c     IGCM GRID:
      ...
      parameter (ilon1_atm=128,ilat1_atm=64)
      integer jg,mg
      parameter(jg=ilat1_atm/2,mg=ilon1_atm)
...
c      call open_file_nc(trim(in_data_runoffname),ncid)
c      call get3d_data_nc(ncid,trim(in_runoff_varname),
c     &          ilon1_atm,ilat1_atm,2,
c     &          runoff_1_igcm,ifail)
c      if (ifail.ne.0) stop 'Problem reading input runoff file'
c      call close_file_nc(trim(in_data_runoffname),ncid)
...
c            do x=1,xlim_1
c              if (runoff_1_igcm(i,j,1).eq.0) then
c                do ii=-x,x
c                  do jj=-x,x
c                    itemp=ii+i
c                    if (itemp.gt.ilon1_atm) itemp=itemp-ilon1_atm
c                    if (itemp.lt.1) itemp=itemp+ilon1_atm
c                    jtemp=jj+j
c                    if (jtemp.gt.ilat1_atm) jtemp=ilat1_atm
c                    if (jtemp.lt.1) jtemp=1
c                    if (runoff_1_igcm(itemp,jtemp,1).ne.0) then
c                      runoff_1_igcm(i,j,1)=runoff_1_igcm(itemp,jtemp,1)
c                      runoff_1_igcm(i,j,2)=runoff_1_igcm(itemp,jtemp,2)
c                    endif
c                  enddo
c                enddo
c              endif
...
c            runoff_1_igcm(i,j,1)=0.0
c            runoff_1_igcm(i,j,2)=0.0
...
c            if ((runoff_1_igcm(i,j,1).eq.0).or.
c     :         (runoff_1_igcm(i,j,2).eq.0)) then
c              print*,'problem a) in 1 at ',i,j
c              stop
c            endif
...
c            if (lsm_1_igcm(runoff_1_igcm(i,j,1),
c     :                      runoff_1_igcm(i,j,2)).ne.0) then
c              print*,'problem b) in 1 at ',i,j
c              stop
c            endif
...
c            if ((runoff_1_igcm(i,j,1).ne.0).or.
c     :         (runoff_1_igcm(i,j,2).ne.0)) then
c              print*,'problem c) in 1 at ',i,j
c              stop
c            endif
...
c      call ininc(out_data_filename_1,
c     :     nmaxdims,ndim,nvar,
c     :     natts,nattsvar,
c     :     vdims,vadims,ndims,
c     :     dimname(1,1),varname(1,1),
c     :     attdimname,attvarname,
c     :     nc(1),iddim(1,1),idvar(1,1))
c      call writedim(nc(1),iddim(1,1),alon1)
c      call writedim(nc(1),iddim(2,1),alat1)
c      call writedim(nc(1),iddim(3,1),months)
c      v=1
c      call writevar(nc(1),
c     :        idvar(loc_dim(varname(v,1),varname(1,1),nall),1),
c     :        runoff_1_igcm )
c      v=2
c      call writevar(nc(1),
c     :        idvar(loc_dim(varname(v,1),varname(1,1),nall),1),
c     :        array_igcm )
c      print*,'written file'
c      call end_netcdf(1)
