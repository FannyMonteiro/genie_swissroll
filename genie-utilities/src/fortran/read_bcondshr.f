...
c          (useful for converting goldstein output)
      ...
      subroutine read_bcondshr(gettype,nmonths,processtype,
     &      dataout,dataouthr,in_filename,
...
c     IGCM GRID:
     ...
     :     aboxedge1hr_lat(ilat1hr_atm+1)
      ...
      integer workhr_igcm(ilon1hr_atm,ilat1hr_atm)
      real dataout(ilon1_atm,ilat1_atm,nmonths)
      real dataout_1_igcm(ilon1_atm,ilat1_atm)
      real dataouthr(ilon1hr_atm,ilat1hr_atm,nmonths)
      real dataouthr_1_igcm(ilon1hr_atm,ilat1hr_atm)
...
c     COPIED FROM INITIALISE_ATMOS.F
      ...
      ax=360.0/real(ilon1hr_atm)
      do i=1,ilon1hr_atm
         aboxedge1hr_lon(i)=(i-1.5)*ax
         alon1hr(i)=(i-1.0)*ax
      end do
      aboxedge1hr_lon(ilon1hr_atm+1)=(mg-0.5)*ax
      call gwtcnr(alat1hr,ilat1hr_atm/2)
      call gwtbox(aboxedge1hr_lat,ilat1hr_atm/2)
...
c        **********************************************
         call awi(in_nlon,in_nlon,in_lonsedge,
     :             in_nlat,in_latsedge,datain_1,work,
     :             ilon1hr_atm,ilon1hr_atm,aboxedge1hr_lon,
     :             ilat1hr_atm,aboxedge1hr_lat,dataouthr_1_igcm,
     :             workhr_igcm,
     :             ifail)
         if (ifail.ne.0) then
            print*,'Interp_to_ocn:  Error in awi,   itype 1 ',ifail
