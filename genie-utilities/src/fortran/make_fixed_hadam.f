      program make_fixed_hadam

c***********************************************************

c     This code can be used to create a forcing field for the
c       genie-fixedatmos module.
c     The forcing field s derived from HadAM runs.

c     I compile with: 
c     ifort -o make_fixed_hadam_NOCVS make_fixed_hadam.f -lnc1 -lutil1 -lnetcdf -ligcm3 -L/home/ggdjl/genie/genie-lib/libnc1 -L/home/ggdjl/genie/genie-lib/libutil1 -L/ -L/home/ggdjl/genie/genie-igcm3/lib -fpp
...
c     DATA:
      ...
      real surftemp_12_igcm(ilon1_atm,ilat1_atm,nmonths)
      ...
      real runoffs_12_igcm(ilon1_atm,ilat1_atm,nmonths)
      real runofff_12_igcm(ilon1_atm,ilat1_atm,nmonths)
...
c     set up work arrays (one everywhere)
      ...
      call read_bconds(2,12,1,surftemp_12_igcm,in_surftemp_filename,
     &      in_surftemp_lonname,in_surftemp_latname,in_surftemp_varname)
     ...
     &      in_stressy_lonname,in_stressy_latname,in_stressy_varname)
       ...
       call read_bconds(2,12,7,runoffs_12_igcm,in_runoffs_filename,
     &      in_runoffs_lonname,in_runoffs_latname,in_runoffs_varname)

       call read_bconds(2,12,7,runofff_12_igcm,in_runofff_filename,
     &      in_runofff_lonname,in_runofff_latname,in_runofff_varname)
...
c     process the data...............
     ...
     :                 (2.5e6)
      surftemp_12_igcm(:,:,:)=surftemp_12_igcm(:,:,:)-273.15
      runoff3_12_igcm(:,:,:)=
     :    (runoffs_12_igcm(:,:,:)+runofff_12_igcm(:,:,:))/(30*60)
...
c     This runoff is directly from Hadam
      do m=1,nmonths
      do j=1,ilat1_atm
        do i=1,ilon1_atm
          if (lsm_1_igcm(i,j).eq.1) then
...
c     12 is for the number of months, 1e12 is for Sverdrups and kg =>m3
      ...
      attvarname(2,1,v)='surftemp from HadAM3'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='prate from HadAM3'
      attvarname(1,2,v)='units'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='sensible heat from HadAM3'
      attvarname(1,2,v)='units'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='latent heat from HadAM3'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='W/m2'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='evaporation (derived) from HadAM3'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='kg/m2/s'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='net surface solar radiation from HadAM3'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='W/m2'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='net surface longwave radiation from HadAM3'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='W/m2'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='x surface wind stress from HadAM3'
      attvarname(1,2,v)='units'
      ...
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='y surface wind stress from HadAM3'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='N/m2'
