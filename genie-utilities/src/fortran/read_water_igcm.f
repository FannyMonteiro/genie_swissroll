...
c     I think it is OK - it is probably the in-line diagnostic
c       which is wrong due to round-off error (DJL, 30/9/2005)

c     I compile with: 
c     ifort -o read_water_igcm_NOCVS read_water_igcm.f -lnc1 -lutil1 -lnetcdf -ligcm3 -L/home/ggdjl/genie/genie-lib/libnc1 -L/home/ggdjl/genie/genie-lib/libutil1 -L/opt/local/intel_fc_81/lib -L/home/ggdjl/genie/genie-igcm3/lib -fpp
...
c     set up work arrays (one everywhere)
      ...
      call get2d_data_nc(ncid,trim(in_precip_varname),
     &          ilon1_atm,ilat1_atm,
     &          datain_1_igcm,ifail)
      if (ifail.ne.0) stop 'Problem reading input precip file'
      precip_1_igcm(:,:)=datain_1_igcm(:,:)

      call get2d_data_nc(ncid,trim(in_evap_varname),ilon1_atm,ilat1_atm,
     &          datain_1_igcm,ifail)
      if (ifail.ne.0) stop 'Problem reading input evap file'
      evap_1_igcm(:,:)=datain_1_igcm(:,:)

      call get2d_data_nc(ncid,trim(in_runoff_varname),
     &          ilon1_atm,ilat1_atm,
     &          datain_1_igcm,ifail)
      if (ifail.ne.0) stop 'Problem reading input runoff file'
      runoff_1_igcm(:,:)=datain_1_igcm(:,:)
...
c     Now calculate some diagnostics:
     ...
     : (precip_1_igcm(i,j)+evap_1_igcm(i,j)+
     :  runoff_1_igcm(i,j))
...
c     12 is for the number of months, 1e12 is for Sverdrups and kg =>m3
      ...
      v=2
      varname(v,1)='at_region'
      vdims(v)=2
      vadims(1,v)=loc_dim('longitude',dimname,nall)
      vadims(2,v)=loc_dim('latitude',dimname,nall)
      nattsvar(v)=2
      attvarname(1,1,v)='long_name'
      attvarname(2,1,v)='Atlantic region'
      attvarname(1,2,v)='units'
      attvarname(2,2,v)='-'
