...
CONTAINS
  ...
  SUBROUTINE sub_glob_avg_weath(dum_sfcatm1,dum_runoff,dum_photo,dum_respveg,dum_sfxrok,dum_sfxatm1)
    ...
    IF (opt_weather_T_Si) THEN
       n = exp(k_T*(loc_avSLT - loc_SLT0)) ! From Brady (1991)
    ...
    IF (opt_weather_R_Si) THEN
       ...
       ELSE
          n = (1.0 + par_k_run*(loc_avSLT - loc_SLT0))**par_beta ! From GEOCARB
       ENDIF
    ...
    ELSE
       IF (opt_outgas_eq_Si.eqv..true.) THEN
          ...
          loc_force_flux_weather_a(ia_PCO2) = - 1.0*(weather_fCaSiO3 + weather_fCaCO3)
       ELSE 
          ! NOTE: '-' because coming out of atmosphere
          loc_force_flux_weather_a(ia_PCO2) = par_outgas_CO2 - 1.0*(2.0*weather_fCaSiO3 + weather_fCaCO3)
       ENDIF
    ...
    ELSE
       ...
       IF (opt_outgas_eq_Si.eqv..true.) THEN
          ...
          loc_force_flux_weather_a(ia_pCO2_13C) = &
               & - 1.0*(fun_calc_isotope_fraction(par_outgas_CO2_d13C,loc_standard)*weather_fCaSiO3 &
               & + fun_calc_isotope_fraction(par_weather_CaCO3_d13C,loc_standard)*weather_fCaCO3) 
       ELSE
          ! NOTE: '-' because coming out of atmosphere
          loc_force_flux_weather_a(ia_pCO2_13C) = &
               & fun_calc_isotope_fraction(par_outgas_CO2_d13C,loc_standard)*par_outgas_CO2 - & 
               & 1.0*(2.0*fun_calc_isotope_fraction(par_outgas_CO2_d13C,loc_standard)*weather_fCaSiO3 &
               & + fun_calc_isotope_fraction(par_weather_CaCO3_d13C,loc_standard)*weather_fCaCO3) 
       ENDIF
       loc_standard = const_standards(ocn_type(io_DIC_13C))
       loc_force_flux_weather_o(io_DIC_13C) = &
            & 2.0*fun_calc_isotope_fraction(par_outgas_CO2_d13C,loc_standard)*weather_fCaSiO3 + &
            & 2.0*fun_calc_isotope_fraction(par_weather_CaCO3_d13C,loc_standard)*weather_fCaCO3
    ENDIF
    ...
    loc_force_flux_weather_o(io_SiO2_30Si) = loc_force_flux_weather_o(io_SiO2_30Si) + &
         & fun_calc_isotope_fraction(par_weather_CaSiO3_fracSi_d30Si,loc_standard)*par_weather_CaSiO3_fracSi*weather_fCaSiO3
