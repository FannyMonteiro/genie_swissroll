...
#
#####################################################################
### SCIPT TO META-CONFIGURE AND RUN CGENIE.swissroll_Nitri ###################
#####################################################################
#
# (0) USER OPTIONS
# ----------------
...
if [ -z "$2" ]; then
    ...
    GOINDIR=$HOME/cgenie.swissroll_Nitri/genie-userconfigs/"$2"
fi
...
if [ -n "$5" ]; then
  RESTARTPATH=$HOME/cgenie.swissroll_Nitri_output/"$5"
fi
#
...
#
OUTPUTPATH=$HOME/cgenie.swissroll_Nitri_output/$RUNID
CONFIGPATH=$HOME/cgenie.swissroll_Nitri/genie-main/configs
#CONFIGNAME=$MODELID"."$RUNID".config"
CONFIGNAME=$RUNID".config"
BINARYPATH=$HOME/cgenie.swissroll_Nitri/genie-main
RESTARTNAME="rst.1"
...
echo ">> Archiving ..."
# archive results
tar cfz $HOME/cgenie.swissroll_Nitri_output/$RUNID.tar.gz $OUTPUTPATH
