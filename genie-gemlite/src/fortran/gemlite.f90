...
subroutine gemlite(    &
  ...
  INTEGER::l,io,is                                                      ! counting indices
  ...
  real::loc_docn_flux                                                   ! ocean tracer change from net external fluxes
  real::loc_tot_A                                                       ! 
  ...
  if (ctrl_update_pCO2) then
     ...
     Do
        ...
        ! CO2 TO BE MOVED BETWEEN OCEAN AND ATMOSPHERE IS EQUAL TO THE PCO2 DIFFERENT BETWEEN OCEAN AND ATMOPSHERE
        ! IN PROPORTION TO ATMOSPHERIC PCO2, AND THEN MULTIPLIED BY THE ATM MASS
        ! I.E. A 10 PPM DIFFERENT COMPARED TO 300 PPM IN THE ATMOSPHERE REQUIRES THE TRANSFER OF 10/300 OF THE ATMM MASS
        ! *BUT* NOTING THAT THE SYSTEM IS NONLINEAR AND HENCE THIS SHOULD NT BE DONE ALL AS ONE ;)
        ...
        loc_DCO2(:,:) = 0.5*loc_ocn_mask(:,:)*(1.0 - phys_ocnatm_seaice(:,:))* &
