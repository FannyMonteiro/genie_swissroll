...
CONTAINS
  ...
  SUBROUTINE sub_save_netcdf_3d_USER()
    ...
    IF (ocn_select(io_PO4) .AND. ocn_select(io_Ca)) THEN
       loc_ijk(:,:,:) = const_real_null
       DO i=1,n_i
          DO j=1,n_j
             DO k=goldstein_k1(i,j),n_k
  ...
  ! ****************************************************************************************************************************** !
  ! INITIALIZE netCDF
  SUBROUTINE sub_init_netcdf (dum_name, dum_iou)
    !-----------------------------------------------------------------------
    !       dummy arguments
    !-----------------------------------------------------------------------
    ...
    call sub_defvar ('grid_level', dum_iou, 2, loc_it, loc_c0, loc_c100, ' ', 'I', &
...
!!$       do n = 1,n_l_ocn
!!$          call sub_defvar ('ocn_'//string_ocn_tname(n), dum_iou, 4, &
...
!!$    end if
  ...
  SUBROUTINE sub_save_netcdf(dum_yr,dum_dd)
    ...
    if(.not. loc_defined) then
       ...
       DO k=n_k,0,-1
          loc_tmp_k(k) = SUM(goldstein_dsc * loc_grid_dz(k+1:n_k+1))
       ENDDO
  ...
  SUBROUTINE sub_save_netcdf_2d()
    ...
    If (ctrl_data_save_slice_misc) then
       ...
       loc_unitsname = 'm s-1'
       call sub_adddef_netcdf(loc_iou,3,'phys_ws','gas transfer windspeed',trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('phys_ws',loc_iou,n_i,n_j,loc_ntrec, &
            & int_phys_ocnatm_timeslice(ipoa_u,:,:)/int_t_timeslice,loc_mask_surf)
    ...
    If (ctrl_data_save_slice_phys_ocn) then
       loc_ij(:,:) = const_real_zero
       DO ip=1,n_phys_ocn
          loc_ij(:,:) = int_phys_ocn_timeslice(ip,:,:,n_k)/int_t_timeslice
          call sub_adddef_netcdf(loc_iou,4,'phys_ocn_'//trim(string_phys_ocn(ip)), &
               & 'full ocean physical properties - '//trim(string_phys_ocn(ip)),' ',const_real_zero,const_real_zero)
          call sub_putvar2d('phys_ocn_'//trim(string_phys_ocn(ip)),loc_iou, &
    ...
    !----------------------------------------------------------------
    If (ctrl_data_save_slice_ocn .AND. ctrl_data_save_slice_diag) then
  ...
  SUBROUTINE sub_save_netcdf_2d_sed()
    ...
    !---------------------------------------------------------------- 
    ! NOTE: exclude dissolved organic matter tracers
    If (ctrl_data_save_slice_ocnsed .AND. ctrl_data_save_slice_misc .AND. ctrl_data_save_derived) then
       DO l=1,n_l_ocn
          io = conv_iselected_io(l)
          is = maxval(maxloc(abs(conv_DOM_POM(:,io))))-1
          if (is == 0) then
                ...
                DO j=1,n_j
                   ...
                   IF (n_k >= loc_k1) THEN
                      ...
                      case (11:20)
                         ...
                         loc_frac = int_ocn_timeslice(io,i,j,n_k)
                         loc_standard = const_standards(ocn_type(io))
                         loc_ij_1(i,j) = fun_calc_isotope_delta(loc_tot,loc_frac,loc_standard,.TRUE.)
                         loc_tot  = int_ocn_timeslice(ocn_dep(io),i,j,loc_k1)
                         loc_frac = int_ocn_timeslice(io,i,j,loc_k1)
                         loc_standard = const_standards(ocn_type(io))
                         loc_ij_2(i,j) = fun_calc_isotope_delta(loc_tot,loc_frac,loc_standard,.TRUE.)
                      end SELECT
                   end IF
                end DO
             end DO
       ...
       DO l=1,n_l_sed
          ...
          CASE (par_sed_type_bio,par_sed_type_abio, &
             ...
             call sub_adddef_netcdf(loc_iou,4,'bio_fpart_'//trim(string_sed(is)), &
                  & 'particulate flux - '//trim(string_sed(is)),loc_unitsname,const_real_zero,const_real_zero)
  ...
  SUBROUTINE sub_save_netcdf_tsi(dum_ntrec, dum_yr, dum_ocn_tot_M, dum_ocn_tot_A, &
    ...
    loc_tmp = fun_calc_isotope_delta(loc_tot,loc_frac,loc_standard,.FALSE.)
