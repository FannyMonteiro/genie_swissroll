    ...
    CASE ( &
       ...
       if(loc_dPO4_2*par_bio_NPdiaz > const_real_nullsmall) then
         loc_frac_N2fix = par_bio_NPdiaz*loc_dPO4_2 / (par_bio_red_POP_PON*loc_dPO4_1 + par_bio_NPdiaz*loc_dPO4_2)
                 !Fanny
                 !print*, &
                 !& ' frac_N2fix=', loc_frac_N2fix,loc_dPO4_1,loc_dPO4_2
...
!!!loc_bio_red_RDOMfrac = (1.0 - 0.5/(par_bio_kT0*exp(loc_TC/par_bio_kT_eT)))*par_bio_red_RDOMfrac
    ...
    DO l=1,n_l_sed
       ...
       ! Correction for higher N:P ratio of N2 fixers - Fanny (June 2010)
       ...
       CASE ( &
        ...
        bio_part(is,dum_i,dum_j,loc_k_mld:n_k) = &
               & bio_part_red(is_POC,is_PON,dum_i,dum_j)*bio_part_red(is_POP,is_POC,dum_i,dum_j)*loc_dPO4_1 &
               & + par_bio_NPdiaz*loc_dPO4_2

                 !Fanny
                 !print*, &
                 !& ' bio_part(PON)=', bio_part(is,dum_i,dum_j,loc_k_mld:n_k),loc_dPO4_1,loc_dPO4_2
       END select
       ! Correction for higher Fe:P ratio for N2 fixers: Calculation from Kfe/Kp of diazo - Fanny (July 2010)
       else if (sed_dep(is) == is_POFe) then
       SELECT CASE (par_bio_prodopt)
       CASE ('3N2T_PNFe_Tdep')
        bio_part(is,dum_i,dum_j,loc_k_mld:n_k) = &
    ...
    DO l=1,n_l_sed
       ...
       !if (sed_dep(is) == is_PON) then
       !          print*, &
       !          & ' loc_bio_uptake(PON)=', loc_bio_uptake(io,loc_k_mld:n_k),conv_sed_ocn(io,is)*bio_part(is,dum_i,dum_j,loc_k_mld:n_k)
       !end if
       ...
       ! check for depletion < 0.0
       ! NOTE: check surface layer only
       ! NOTE: DO NOT apply to nutrients (nutrient limitation is treated seperately)
       select case (sed_type(is))
       case (par_sed_type_POM,par_sed_type_CaCO3,par_sed_type_opal)
          loc_tot_i = conv_sed_ocn_i(0,is)
          do loc_i=1,loc_tot_i
             io = conv_sed_ocn_i(loc_i,is)
             if (conv_sed_ocn(io,is)*bio_part(is,dum_i,dum_j,n_k) > ocn(io,dum_i,dum_j,n_k)) then
                select case (io)
                case (io_PO4,io_NO3,io_NH4,io_SiO2,io_Fe)
                   ! (nothing)
                case default
    ...
    CASE ( &
       ...
       ! New NH4 production - Fanny (June 2015)
       if ((ocn(io_NH4_new,dum_i,dum_j,n_k) >= 0) .and. (ocn(io_NH4,dum_i,dum_j,n_k) >= 0) .and. (loc_bio_uptake(io_NH4,n_k) >=0)) then
       ...
       ! New NO3 production - Fanny (Sep 2015)
    ...
    ! Setting NH4_new to NH4 and NO3_new to NO3 below a depth threshold set here to k=n_k - Fanny (June 2015)
    if (ocn_select(io_NO3).and.ocn_select(io_NH4)) then
     ...
     SUBROUTINE sub_calc_bio_remin_oxidize_NH4(dum_i,dum_j,dum_k1,dum_dtyr)
       ...
       DO k=n_k,dum_k1,-1
	     ...
	     ! Second order equation of enzyme kinetics which accounts for both O2 and NH4 limitations on nitrification
             ...
             !! Nitrification of NH4_new into NO3_new - Fanny (Oct 2015)
             !if (k <= 3) then
