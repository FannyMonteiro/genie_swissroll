...
CONTAINS
  ...
  SUBROUTINE sub_load_goin_sedgem()
    ...
    if (ctrl_debug_init > 0) then
       ...
       print*,'Save threshold of accumulated sediment change (cm)  : ',par_sed_save_top_INTdth
       ! --- I/O: MISC -------------------------------------------------------------------------------------------------------------- !
       print*,'--- I/O: MISC --------------------------------------'
    ...
    ! set ash event flux (g cm-2 kyr-1)
    par_sed_ashevent_fash = 10.0
...
!!$    call sub_closefile(loc_iou)
  ...
  SUBROUTINE sub_data_save_seddiag_GLOBAL(dum_dtyr,dum_sfcsumocn)
    ...
    call check_iostat(ios,__LINE__,__FILE__)
    write(unit=out,fmt='(A28,I6)',iostat=ios) &
         & ' Active # reef grid pts    :',int(sum(loc_mask_reef_active(:,:)))
    call check_iostat(ios,__LINE__,__FILE__)
    write(unit=out,fmt='(A28,e14.6,A3)',iostat=ios) &
         & ' Active reef area          :',sum(loc_mask_reef_active(:,:)*phys_sed(ips_A,:,:)),'m2'
    call check_iostat(ios,__LINE__,__FILE__)
    Write(unit=out,fmt=*) '---------------------------------'
    ! local variables 
