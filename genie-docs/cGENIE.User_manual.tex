...
\begin{compactenum}
	...
	\begin{compactitem}
		...
		\item Plotting netCDF output using 'Panoply'.
		\item Plotting netCDF output using the MATLAB scripts in \texttt{genie-matlab}.
	\end{compactitem}
\end{compactenum}
...
The equivalents of \texttt{bg\_par\_infile\_sig\_name} and \texttt{bg\_par\_infile\_slice\_name} for SEDGEM and ROKGEM are \texttt{xx\_par\_output\_years\_file\_0d} and \texttt{xx\_par\_output\_years\_file\_2d} respectively, where \texttt{xx} is \texttt{sg} for SEDGEM and \texttt{rg} for ROKGEM. These specify files in genie-*gem/data/input where years are listed for 0D (time-series) and 2D (spatial time slice - netCDF) output to be generated at. Note that unlike BIOGEM, the data saved \textit{do} represent discrete values in time and \textit{not} averages.

If running a multi-stage experiment, for your BIOGEM, SEDGEM, ROKGEM and ENTS output (both 0D and 2D) not to be overwritten each time the model is restarted you need to set \texttt{bg\_opt\_append\_data=.TRUE.} and \texttt{ents\_opt\_append\_data=.TRUE.} in your config file (\texttt{sg\_ctrl\_append\_data=.TRUE.} and \texttt{\seqsplit{rg\_opt\_append\_data=.TRUE.}} are set by default).
...
You can also view netCDF files from MUTLAB, for which a number of plotting functions have been written (see below).
...
There are several functions available for plotting \textit{BIOGEM} and \textit{SEDGEM} \textit{netCDF} output.
...
\begin{compactenum}
	...
	\\If the mask is just of a single basin such as the Atlantic (\texttt{mask\_worjh2\_Atlantic.dat}), the result is the Atlantic meridional cross-sectional mean. Masks can also be constructed that are only a single cell 'wide' longtudinally, but which take a meandering path following an ocean transect\footnote{e.g., as in: \texttt{mask\_worjh2\_GEOSECS\_WATL.dat}}. The trivial usage would be to construct a mask consisting of a vertical slice with equal \texttt{i} -- the result is then in effect exactly the same as using \texttt{plot\_fields\_biogem\_3d\_i.m}.
	\\The mask is a 2-D array of model grid points (on the BIOGEM grid). A value of '1' represents a vertical column of ocean cells to include, whereas a value '0' will exclude all cells in the water column at that particular grid point.
		\item \texttt{plot\_fields\_biogem\_3d\_k.m} -- Plot a horizontal slice through the ocean (i.e., all cells having the same \texttt{k} (vertical) coordinate) from: \texttt{fields\_biogem\_2d.nc}.\footnote{For the 8-layer version of the ocean circulation model, legal values for \texttt{k} range from 1 through 8, with \texttt{k=8} representing the uppermost (surface layer), while for the 16-level version, legal values for \texttt{k} are 1-16.}
		\item \texttt{plot\_fields\_sedgem\_2d.m} -- plot a 2-D field from: \texttt{fields\_sedgem\_2d.nc}
\end{compactenum}

All 4 plotting routines share the same basic format of input parameters, which start:
\begin{compactitem}
\item \texttt{PATH} -- is the relative pathway to the directory location of the experiments. For example, if all the individually-named experiment directories are located in a directory named \texttt{cgenie\_output} (as per how results are saved by default by the model), and this directory is located in the same directory as the MATLAB function is being run, parameter \texttt{PATH} takes the value: \texttt{'cgenie\_output'}.\footnote{Note that as a string value, the inverted commas are important.} If the experiment directory was located in the same directory as the MATLAB function, a null value must be passed, i.e., \texttt{''}.
\item \texttt{EXP1} -- is the name of the 1st (main) experiment.
\item \texttt{EXP2} -- is the name of the 2nd (optional) experiment. If no second experiment is selected, then a null string value must be passed, i.e., \texttt{''}.
\item \texttt{PT1} -- is the value of the 1st (main) time-slice. If no valid variable value is given, a list of valid variable names will be printed out.
\item \texttt{PT2} -- is the value of the 2nd (optional) time-slice. If no second time-slice is selected, then leave completely blank (not even a pair of inverted commas).
...
\end{compactitem}
The main exception to this is \texttt{plot\_fields\_sedgem\_2d.m}, because \textit{SEDGEM} only saves a (single) time-slice at the very end of an experiment.
In addition, \texttt{plot\_fields\_biogem\_3d\_i.m} is currently missing the second (optional) variable parameter for input.
By setting optional experiments, time-slices, or variables, a difference map will be created.
Again, with the exception of \texttt{plot\_fields\_sedgem\_2d.m}, all functions then require:
\begin{compactitem}
\item \texttt{MASK} -- is the name of an optional (2D) mask. A null string (\texttt{''}) must be passed if no mask is requested.
\item \texttt{PK,PI,PKMAX} -- the depth (\texttt{k}) level (\texttt{plot\_fields\_biogem\_3d\_k.m}) or longitude (i) slice \\
(\texttt{plot\_fields\_biogem\_3d\_i.m}) to be plotted. For \texttt{fields\_biogem\_2d.nc}, this value also provides an alternative way of creating a mask, and only values of \texttt{k} less than of equal to the passed value will be plotted.
...
\end{compactitem}
All functions then have a common plotting scale control:
\begin{compactitem}
\item \texttt{CSCALE} -- is the scale factor for the plot. For example, to plot in micro molar (umol kg-1) units, enter; \texttt{1e-6}. The plot is auto-scaled if a value of zero (\texttt{0.0}) is entered.
\item \texttt{CMIN} -- is the minimum scale value.
\item \texttt{CMAX} -- is the maximum scale value.
\item \texttt{CN} -- is the number of (contour) intervals between minimum and maximum scale values.
\end{compactitem}
Finally, for all functions:
\begin{compactitem}
...
The following script carries out the basic configuration tasks and packages up the results -- \texttt{runcgenie.sh} (similar to \texttt{old\_rungenie.sh} before. `CC' means it includes climate and the (ocean) carbon cycle. By default it has 96 time steps per year compared with a 100 previously.
-- \texttt{runcgenie.sh} is held under svn and resides in the directory \texttt{\~{}/cgenie/genie-main}, from where experiments are run.
 Note the file \texttt{runcgenie.sh} \textbf{MUST} have executable permissions (\texttt{chmod u+x runcgenie.sh}).

	\item   For running GENIE-1 via the `\texttt{runcgenie.sh}' script, you must create the following two directories:
  \vspace{-5.5pt}\begin{verbatim}
	   ~/cgenie_archive
   ~/cgenie_log
  \end{verbatim}\vspace{-11pt}
...
./runcgenie.sh cgenie\_eb\_go\_gs\_ac\_bg\_itfclsd\_16l\_JH\_BASEFe / EXAMPLE\_worjh2\_Fe\_SPIN 11
...
bg\_par\_forcing\_name=``worjh2\_preindustrial\_FeMahowald2006"
        ...
        Because the way the cluster is set up you cannot submit runcgenie.sh from genie-main. Instead use subCCgenie.sh (make executable!) to run from \$HOME.
        
For example:
  qsub -q kitten.q -j y -o cgenie\_log -S /bin/bash subCCgenie.sh\\
  cgenie\_eb\_go\_gs\_ac\_bg\_itfclsd\_16l\_JH\_BASEFe / EXAMPLE\_worjh2\_Fe\_SPIN 51
